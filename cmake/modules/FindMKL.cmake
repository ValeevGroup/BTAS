...
# this invokes FIND_LIBRARY, but only looks for shared versions of functions that do not include "mkl  in their name, .e.g libpthread libm and libdl
macro(BTAS_FINDMKL_FIND_LIBRARY)
  if (NOT ${ARGC} EQUAL 3)
    message(FATAL_ERROR "BTAS_FINDMKL_FIND_LIBRARY expects 3 arguments")
  endif()
  if (NOT (${ARGV2} MATCHES mkl))
    set(_orig_CMAKE_FIND_LIBRARY_SUFFIXES ${CMAKE_FIND_LIBRARY_SUFFIXES})
    string(REPLACE "." "" _static_suf "${CMAKE_STATIC_LIBRARY_SUFFIX}")
    list(FILTER CMAKE_FIND_LIBRARY_SUFFIXES EXCLUDE REGEX "\.${_static_suf}")
    find_library(${ARGN})
...
IF (NOT MKL_FOUND)
  ...
  ELSE (WIN32)
    if (DEFINED ENV{MKLROOT})
      ...
      get_filename_component(DEFAULT_INTEL_COMPILER_DIR "${DEFAULT_INTEL_MKL_DIR}/.." REALPATH)
    else()
      ...
      IF(_libraries_work)
        IF(${_library} MATCHES "omp")
          ...
          ELSEIF(${_library} MATCHES "iomp")
            ...
            BTAS_FINDMKL_FIND_LIBRARY(${_prefix}_${_library}_LIBRARY NAMES ${_library})
            SET(_openmp_library "${${_prefix}_${_library}_LIBRARY}")
          ELSE()
        ...
        ELSE()
          ...
          BTAS_FINDMKL_FIND_LIBRARY(${_prefix}_${_library}_LIBRARY NAMES ${lib_names})
        ENDIF()
    ...
    FOREACH(mkl64 ${mkl64s} "_core" "")
      FOREACH(mkls ${mklseq} "")
        IF (NOT MKL_LAPACK_LIBRARIES)
          BTAS_FINDMKL_FIND_LIBRARY(MKL_LAPACK_LIBRARIES NAMES "mkl_lapack${mkl64}${mkls}")
          MARK_AS_ADVANCED(MKL_LAPACK_LIBRARIES)
        ENDIF (NOT MKL_LAPACK_LIBRARIES)
        IF (NOT MKL_SCALAPACK_LIBRARIES)
          BTAS_FINDMKL_FIND_LIBRARY(MKL_SCALAPACK_LIBRARIES NAMES "mkl_scalapack${mkl64}${mkls}")
          MARK_AS_ADVANCED(MKL_SCALAPACK_LIBRARIES)
        ENDIF (NOT MKL_SCALAPACK_LIBRARIES)
        IF (NOT MKL_SOLVER_LIBRARIES)
          BTAS_FINDMKL_FIND_LIBRARY(MKL_SOLVER_LIBRARIES NAMES "mkl_solver${mkl64}${mkls}")
          MARK_AS_ADVANCED(MKL_SOLVER_LIBRARIES)
        ENDIF (NOT MKL_SOLVER_LIBRARIES)
        IF (NOT MKL_CDFT_LIBRARIES)
          BTAS_FINDMKL_FIND_LIBRARY(MKL_CDFT_LIBRARIES NAMES "mkl_cdft${mkl64}${mkls}")
