...
public:
  ...
  private:
   ...
   void allocate(size_type n) {
     assert(n <= allocator_traits::max_size(*this));
     data_._M_start = allocator_traits::allocate(*this, n);
     data_._M_finish = data_._M_start + n;
   }

   void deallocate() {
     if (!data_.empty())
       allocator_traits::deallocate(*this, data_._M_start, data_.size());
   ...
   construct(size_type n)
   {
     auto ptr = data_._M_start;
       do
       {
           allocator_traits::construct(*this, ptr);
           ++ptr;
           --n;
       } while (n > 0);
   }
   ...
   construct(size_type n, const_reference x)
   {
     ...
     do
     {
       allocator_traits::construct(*this, ptr, x);
       ++ptr;
       --n;
     } while (n > 0);
   }
   ...
   construct(const InputIterator& begin, const InputIterator& end) {
     ...
     for(auto i = begin; i != end; ++i) {
       allocator_traits::construct(*this, ptr, *i);
       ++ptr;
     }
   }
};
