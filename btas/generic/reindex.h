#ifndef __BTAS_REINDEX_H
#define __BTAS_REINDEX_H 1
...
struct __NDloop__reindex
{
   ...
   __NDloop__reindex (const _T*& pX, _T*& pY, size_type addrX, const std::array<size_type, _N>& strX, const std::array<size_type, _N>& shapeY)
   {
      for (size_type i = 0; i < shapeY[_I-1]; ++i)
      {
         __NDloop__reindex<_I+1, _N> loop(pX, pY, addrX+i*strX[_I-1], strX, shapeY);
      }
   }
};
...
struct __NDloop__reindex<_N, _N>
{
   /// loop upon construction
...
{
   __NDloop__reindex<1, _N> loop(pX, pY, 0, strX, shapeY);
}

} // namespace btas

#endif // __BTAS_REINDEX_H
