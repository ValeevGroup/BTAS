...
namespace btas {
  ...
  void swap_to_first(Tensor &A, size_t mode, bool is_in_front = false, bool for_ALS_update = true) {
    ...
    // will preserve order of the dimensions.
      ...
      for (size_t i = 0; i <= mode; i++) rows *= A.extent(i);
      cols = size / rows;
      dtype *data_ptr = A.data();
      ...
      for (ind_t i = 0; i < cols; i++) {
        ...
        imatcopy('R', 'T', in_rows, in_cols, one, data_ptr, in_cols, in_rows);
      }
      data_ptr = A.data();
  ...
  void swap_to_back(Tensor &A, size_t mode, bool is_in_back = false) {
    ...
    if (mode > ndim) BTAS_EXCEPTION_MESSAGE(__FILE__, __LINE__, "mode > A.rank(), mode out of range");
    if (mode == ndim - 1) return;
