...
namespace btas {
  ...
  // the result tracks constness at compile time (compare to TensorRWView)
  // if std::is_const<_T>::value is true, _T::const_storage_iterator, otherwise _T::storage_iterator
  template<typename _T>
  using TensorViewOf = TensorView<typename Nref<_T>::value_type,
                                  ...
                                  typename std::conditional<std::is_const<Nref<_T>>::value,
                                                            typename Nref<_T>::const_storage_iterator,
                                                            typename Nref<_T>::storage_iterator
                                                           >::type
                                 >;

  template<typename _T, typename _Permutation>
  TensorViewOf<_T> permute(_T&& t, _Permutation p) {
    return make_view(permute(t.range(), p), t.storage());
  }

  template<typename _T, typename _U>
  TensorViewOf<_T> permute(_T&& t, std::initializer_list<_U> p) {
    return make_view(permute(t.range(), p), t.storage());
  }

  template<typename _T>
  TensorViewOf<_T> diag(_T&& T) {
    return make_view(diag(T.range()), T.storage());
  }

  template<typename _T, typename ArrayType>
  TensorViewOf<_T> tieIndex(_T&& T, const ArrayType& inds) {
    return make_view(tieIndex(T.range(), inds), T.storage());
  }

  template<typename _T, typename ... _args>
  TensorViewOf<_T> tieIndex(_T&& T, size_t i0, const _args&... rest) {
  ...
  template<typename _T>
  TensorViewOf<_T> group(_T&& T, size_t istart, size_t iend) {
    return make_view(group(T.range(), istart, iend), T.storage());
  }

  template<typename _T>
  TensorViewOf<_T> flatten(_T&& T) {
