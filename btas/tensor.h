...
namespace btas {
  ...
  */
  ...
  class Tensor {
   public:
    static_assert(std::is_same<_T, typename _Storage::value_type>::value,
                  "Tensor<_T,_Range,_Storage> instantiated but _T != _Storage::value_type");
    ...
    template <typename U, class RangeU, class StorageU>
   ...
   public:
    ...
    explicit Tensor(const size_type& first, const _args&... rest) : range_(range_type(first, rest...)) {
    ...
    explicit Tensor(const Range& range, typename std::enable_if<btas::is_boxrange<Range>::value>::type* = 0)
    ...
    template <typename Range>
    Tensor(const Range& range, value_type v, typename std::enable_if<btas::is_boxrange<Range>::value>::type* = 0)
        : range_(range.lobound(), range.upbound()) {
      array_adaptor<storage_type>::resize(storage_, range_.area());
    ...
    /// @sa Tensor::clone()
    Tensor(const Tensor& x) : range_(x.range()), storage_(x.storage_) {}

    /// move constructor
    Tensor(Tensor&& x) : range_(std::move(x.range())), storage_(std::move(x.storage_)) {}
    ...
    std::conditional_t<is_complex_type_v<_T>, Tensor, const Tensor&> conj() const {
      if constexpr (is_complex_type_v<_T>) {
        ...
        auto self_ptr = data();
        auto self_size = size();
        for (auto i = 0; i < self_size; ++i) {
          *(conj_ptr + i) = btas::impl::conj(*(self_ptr + i));
        }
        return conjT;
      } else {
        return *this;
    ...
    template <class _Tensor, class = typename std::enable_if<
                                 ...
                                 not std::is_same<typename _Tensor::storage_type, Tensor::storage_type>::value>::type>
    Tensor& operator=(const _Tensor& x) {
      using std::begin;
      using std::cbegin;
      using std::cend;
      using std::end;
    ...
    template <class _Tensor, class = typename std::enable_if<is_boxtensor<_Tensor>::value>::type,
              class = typename std::enable_if<
                  std::is_same<typename _Tensor::storage_type, Tensor::storage_type>::value>::type>
    Tensor& operator=(const _Tensor& x) {
      using std::begin;
      using std::cbegin;
      using std::cend;
    ...
    template <
        typename Scalar,
        typename = typename std::enable_if<not std::is_same<typename std::decay<Scalar>::type, Tensor>::value &&
                                           not btas::is_boxtensor<typename std::decay<Scalar>::type>::value>::type,
        typename = btas::void_t<decltype(static_cast<typename storage_type::value_type>(std::declval<Scalar>()))>>
    ...
    size_type rank() const { return range_.rank(); }

    /// \return range object
    const range_type& range() const { return range_; }

    /// \return range's extent object
    typename range_type::extent_type extent() const { return range_.extent(); }

    /// \return extent of range along dimension \c d
    typename range_type::extent_type::value_type extent(size_t d) const { return range_.extent(d); }
    ...
    template <typename Index>
    typename std::enable_if<is_index<Index>::value, const_reference>::type operator()(const Index& index) const {
    ...
    template <typename Index>
    typename std::enable_if<is_index<Index>::value, reference>::type operator()(const Index& index) {
      return storage_[range_.ordinal(index)];
    }
    ...
    template <typename Range>
    void resize(const Range& range, typename std::enable_if<is_boxrange<Range>::value, Enabler>::type = Enabler()) {
    ...
    void resize(
        const Extent& extent,
        typename std::enable_if<is_index<Extent>::value && not is_boxrange<Extent>::value, Enabler>::type = Enabler()) {
    ...
    template <typename U>
    TensorView<value_type, range_type, const storage_type> slice(std::initializer_list<Range1d<U>> range1s) const {
      return __make_cview<value_type, range_type, storage_type, btas::TensorViewPolicy<>>(this->range().slice(range1s),
                                                                                          this->storage());
    }
  ...
  auto range(const _Tensor& t) -> decltype(t.range()) {
    return t.range();
  }

  /// maps Tensor -> Range extent
  template <class _Tensor, class = typename std::enable_if<btas::is_boxtensor<_Tensor>::value>::type>
  auto extent(const _Tensor& t) -> decltype(t.range().extent()) {
    return t.range().extent();
  }

  /// maps Tensor -> Range rank
  template <class _Tensor, class = typename std::enable_if<btas::is_boxtensor<_Tensor>::value>::type>
  auto rank(const _Tensor& t) -> decltype(t.rank()) {
    return t.rank();
  }
  ...
  template <class _Tensor1, class _Tensor2, class = typename std::enable_if<btas::is_boxtensor<_Tensor1>::value>::type,
            class = typename std::enable_if<btas::is_boxtensor<_Tensor2>::value>::type>
  bool operator==(const _Tensor1& t1, const _Tensor2& t2) {
    using std::cbegin;
    using std::cend;
    if (btas::range_traits<std::decay_t<decltype(t1.range())>>::order ==
            btas::range_traits<std::decay_t<decltype(t2.range())>>::order &&
        is_contiguous(t1.range()) && is_contiguous(t2.range()))  // plain Tensor
      return congruent(t1.range(), t2.range()) &&
             std::equal(cbegin(t1.storage()), cend(t1.storage()), cbegin(t2.storage()));
    else {  // not plain, or different orders
      auto cong = congruent(t1.range(), t2.range());
      if (not cong) return false;
      typedef TensorView<typename _Tensor1::value_type, typename _Tensor1::range_type,
                         const typename _Tensor1::storage_type>
          cview1;
      typedef TensorView<typename _Tensor2::value_type, typename _Tensor2::range_type,
                         const typename _Tensor2::storage_type>
          cview2;
      cview1 vt1(t1);
      cview2 vt2(t2);
  ...
  template <typename _T, size_t _N, blas::Layout _Order = blas::Layout::RowMajor,
...
namespace boost {
  namespace serialization {
    ...
    /// boost serialization
    template <class Archive, typename _T, class _Range, class _Storage>
